...
內建能力包含帳密與 Google 登入、Session/refresh、OAuth 2.1／OpenID Connect、Remote MCP、使用者與 RBAC、資產儲存、CMS、共用 Svelte UI、Storybook，以及唯一一套 Drizzle migration history。
...
│   ├── content/{assets,cms}
│   └── infra/{database,logger,mail,scheduling,mcp-server,oauth-server}
├── types/{shared,identity,content}  # 無 framework 的純型別
├── browser/sdk                     # Browser-only、UI framework-neutral
├── svelte/{ui,service-ui}          # Svelte-only UI
...
- [如何新增 Remote MCP Tool](doc/onboarding/how-to-add-remote-mcp-tool.md)
