...
import { cmsTags, postContents, postDailyViews, postTags, posts } from './cms.schema.js';
import { assets } from '@platform/nest-content-assets';
import type {
...
import { parsePostSorts, type PostSortProperty } from './list-posts-query.js';
...
export class CmsService implements CmsSearchCapability {
    ...
    private toCmsPostStatus(status: string): CmsPostStatus {
        if (!['draft', 'published', 'archived'].includes(status)) {
            throw new BadRequestException(`Invalid persisted CMS post status: ${status}`);
        }
        return status as CmsPostStatus;
    }
    ...
    private parseAssetIdFromCmsAssetPath(input: string) {
        const matched = input.match(/\/cms\/assets\/([0-9a-fA-F-]{36})\/(?:public|url)(?:[/?#]|$)/);
    ...
    private async resolveAssetProxyUrl(
        ...
        const [asset] = await this.db
            .select({ id: assets.id })
            .from(assets)
            .where(and(eq(assets.storageKey, storageKey), ne(assets.status, 'deleted')))
            .limit(1);
    ...
    private async normalizeBodyAssetUrls(
        ...
        if (Array.isArray(node)) {
            return Promise.all(node.map((item) => this.normalizeBodyAssetUrls(item, cache)));
        }
        ...
        for (const [key, value] of Object.entries(current)) {
            if (key === 'attrs' && value && typeof value === 'object') {
                ...
                if (nodeType === 'image' && typeof attrs.src === 'string') {
                    attrs.src = (await this.resolveAssetProxyUrl(attrs.src, cache)) ?? attrs.src;
                }

                if (nodeType === 'linkPreview' && typeof attrs.image === 'string') {
                    attrs.image = (await this.resolveAssetProxyUrl(attrs.image, cache)) ?? attrs.image;
                }
    ...
    private pickMetaContent(html: string, key: string, keyAttr: 'property' | 'name') {
        ...
        return html.match(firstPattern)?.[1]?.trim() || html.match(secondPattern)?.[1]?.trim() || null;
    }
    ...
    private buildExcerpt(body: unknown, fallbackTitle = 'Untitled') {
        const text = this.extractTextFromTiptapNode(body).join(' ').replace(/\s+/g, ' ').trim();
        if (!text) return fallbackTitle;
        return text.length > 160 ? `${text.slice(0, 157)}...` : text;
    }

    async createPost(userId: number, title: string, locale = 'en'): Promise<CmsPost> {
        // 1. Create Post
    ...
    async getPost(id: string, locale: string): Promise<CmsPost> {
        ...
        return {
            ...post,
            status: this.toCmsPostStatus(post.status),
    ...
    async updatePostContent(
        ...
        const normalizedData = {
            ...
            linkPreviewDescription: this.normalizeOptionalText(data.linkPreviewDescription),
    ...
    async listPosts(page = 1, limit = 10, locale = 'en', options: ListPostsOptions = {}): Promise<ListPostsResponse> {
        ...
        if (normalizedQuery) {
            ...
            conditions.push(or(
                ilike(posts.slug, keyword),
                ilike(postContents.title, keyword),
            ));
        }
        ...
        const sortColumns: Record<PostSortProperty, any> = {
        ...
        const orderBy = [
            ...parsePostSorts(options.sort).map((sort) =>
                sort.direction === 'asc'
        ...
        const enriched = results.map((post) => ({
            ...post,
            status: this.toCmsPostStatus(post.status),
    ...
    async listTags(query?: string): Promise<ListTagsResponse> {
        ...
        const results = await this.db
            ...
            .where(whereClause)
            .groupBy(cmsTags.id, cmsTags.name, cmsTags.slug, cmsTags.createdAt, cmsTags.updatedAt)
            .orderBy(asc(cmsTags.name));
    ...
    async listPublicPosts(page = 1, limit = 10, locale = 'en', options: ListPublicPostsOptions = {}): Promise<ListPublicPostsResponse> {
        const offset = (page - 1) * limit;
        const normalizedQuery = options.query?.trim();
        ...
        const normalizedRows = await Promise.all(rows.map(async (post) => ({
            ...
            coverImage: (await this.resolveAssetProxyUrl(post.coverImage, coverImageCache)) ?? null,
    ...
    async getPublicPostBySlug(slug: string, locale = 'en'): Promise<PublicCmsPostDetail> {
        ...
        let [content] = await this.db
            .select()
            .from(postContents)
            .where(and(eq(postContents.postId, post.id), eq(postContents.locale, locale)))
            .limit(1);
        ...
        if (!content && locale !== 'en') {
            [content] = await this.db
                .select()
                .from(postContents)
                .where(and(eq(postContents.postId, post.id), eq(postContents.locale, 'en')))
                .limit(1);
    ...
    async getPublicHome(locale = 'en', latestLimit = 6, hotTagLimit = 4, hotPostPerTag = 3): Promise<PublicHomeResponse> {
        const latest = await this.listPublicPosts(1, latestLimit, locale, { sort: 'latest' });
        ...
        const hotTagRows = await this.db
            ...
            .orderBy(desc(sql`coalesce(sum(${posts.viewCount}), 0)`), asc(cmsTags.name))
            .limit(hotTagLimit);

        const hotTags = await Promise.all(
            hotTagRows.map(async (tag) => {
                const taggedPosts = await this.listPublicPosts(1, hotPostPerTag, locale, {
                    tagSlug: tag.slug,
                    sort: 'popular',
                });
    ...
    async getDashboardAnalytics(locale = 'en', days = 14, topLimit = 5): Promise<CmsDashboardAnalyticsResponse> {
        const normalizedDays = Number.isFinite(days) ? Math.min(Math.max(Math.floor(days), 1), 90) : 14;
        const normalizedTopLimit = Number.isFinite(topLimit) ? Math.min(Math.max(Math.floor(topLimit), 1), 20) : 5;
        ...
        const dailyRows = await this.db
            ...
            .from(postDailyViews)
            .innerJoin(posts, and(eq(posts.id, postDailyViews.postId), eq(posts.status, 'published')))
        ...
        const topPosts = await this.db
            ...
            .orderBy(desc(posts.viewCount), desc(sql`coalesce(${posts.publishedAt}, ${posts.updatedAt})`))
    ...
    async updateTag(id: string, data: { name?: string }) {
        const [existing] = await this.db.select().from(cmsTags).where(eq(cmsTags.id, id)).limit(1);
        if (!existing) {
            throw new NotFoundException('Tag not found');
        }

        const nextName = data.name ? this.normalizeTagName(data.name) : existing.name;
        if (!nextName) {
            throw new BadRequestException('Tag name is required');
        }
    ...
    async deleteTag(id: string) {
        const [existing] = await this.db.select({ id: cmsTags.id }).from(cmsTags).where(eq(cmsTags.id, id)).limit(1);
        if (!existing) {
            throw new NotFoundException('Tag not found');
        }
    ...
    async getLinkPreview(inputUrl: string) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new BadRequestException(`Unable to fetch URL (${response.status})`);
            }
            ...
            const canonicalUrl = this.pickMetaContent(html, 'og:url', 'property') || normalizedUrl.toString();
            const resolvedImage = this.resolveAgainstBase(image, normalizedUrl);
            const resolvedCanonicalUrl = this.resolveAgainstBase(canonicalUrl, normalizedUrl) || normalizedUrl.toString();
    ...
    async updatePostTags(id: string, tagIds: string[], locale = 'en') {
        const [post] = await this.db.select({ id: posts.id }).from(posts).where(eq(posts.id, id));
        if (!post) {
            throw new NotFoundException('Post not found');
        }
