...
import { AuthGuard } from '@platform/nest-identity-auth';
...
export class AccessControlController {
    ...
    @Get('roles')
    @RequirePermissions(PermissionCodes.Roles.Read)
    ...
    async getRoles(@Query() query: { q?: string; kind?: string; sort?: string | string[] }) {
        return this.service.getRoles({
            search: query.q,
            kind: query.kind === 'system' || query.kind === 'custom' ? query.kind : undefined,
            sort: query.sort,
        });
    }

    @Post('roles')
    @RequirePermissions(PermissionCodes.Roles.Create)
    ...
    @Put('roles/:id')
    @RequirePermissions(PermissionCodes.Roles.Update)
    ...
    @Delete('roles/:id')
    @RequirePermissions(PermissionCodes.Roles.Delete)
    ...
    @Get('permissions')
    @RequirePermissions(PermissionCodes.Permissions.Read)
    ...
    @RequirePermissions(PermissionCodes.Roles.ManagePermissions)
    ...
    @RequirePermissions(PermissionCodes.Users.ManageRoles)
    ...
    @ApiQuery({ name: 'roleId', required: false, isArray: true, description: 'Match any assigned role ID' })
    @ApiQuery({ name: 'sort', required: false, isArray: true, description: 'Ordered sort rules' })
    ...
    async getUsers(@Query() query: { page?: string; limit?: string; q?: string; roleId?: string | string[]; sort?: string | string[] }) {
        return this.service.getUsers({
            page: query.page ? parseInt(query.page) : undefined,
            limit: query.limit ? parseInt(query.limit) : undefined,
            search: query.q,
            roleIds: query.roleId ? (Array.isArray(query.roleId) ? query.roleId : [query.roleId]) : undefined,
            sort: query.sort,
        });
    }

    @Post('users')
    @RequirePermissions(PermissionCodes.Users.Create)
    ...
    @Delete('users/:userId')
    @RequirePermissions(PermissionCodes.Users.Delete)
    ...
    @Put('users/:userId')
    @RequirePermissions(PermissionCodes.Users.Update)
