...
it('should generate correct error message', () => {
	expect(formatError('INVALID_INPUT')).toMatchInlineSnapshot(`"Error: Invalid input provided"`);
});
...
```typescript
// ✅ 真实场景：类型安全的配置对象
declare function createConfig<const T extends Record<string, unknown>>(config: T): T;
