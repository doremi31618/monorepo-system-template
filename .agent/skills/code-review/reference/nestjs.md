...
describe('CreateUserHandler', () => {
	...
	it('creates a user', async () => {
		const id = await handler.execute(new CreateUserCommand('user@example.com', 'Alice'));
		expect(id).toBeDefined();
	});

	it('rejects duplicate email', async () => {
		await handler.execute(new CreateUserCommand('user@example.com', 'Alice'));
		await expect(handler.execute(new CreateUserCommand('user@example.com', 'Bob'))).rejects.toThrow(
			'already exists'
		);
	});
});
