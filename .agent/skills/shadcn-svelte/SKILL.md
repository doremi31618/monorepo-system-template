...
## Data Discovery Controls

When implementing search, filter, sort, table toolbars, query builders, or saved views:

1. Read
   [`../ui-ux-pro-max/references/desktop-data-interactions.md`](../ui-ux-pro-max/references/desktop-data-interactions.md)
   completely before choosing components.
2. Preserve separate search, filter, and sort semantics inside one coherent data-view toolbar.
3. Prefer `Popover` + `Command`/`Combobox` for compact desktop rule editing and `Sheet` for
   narrow screens.
4. Keep active rule counts visible with a label or `Badge`.
5. Keep complete rules URL-backed and exclude incomplete drafts from result queries.
6. Do not default to a separate Card full of form fields or require a desktop Apply button.

## Action Width and Alignment

When implementing buttons, form actions, card actions, or dialog footers:

1. Read
   [`../ui-ux-pro-max/references/action-sizing.md`](../ui-ux-pro-max/references/action-sizing.md)
   completely.
2. Keep desktop buttons content-sized by default.
3. Use full width only for a justified narrow-form, mobile, or single-action pattern.
4. Check parent flex and grid alignment before adding or removing `w-full`.
5. Do not place a Button directly in a stretching `flex-col` or grid container. Wrap actions in a
   `flex flex-wrap items-center gap-2` row or apply `self-start`/`self-end` intentionally.
6. Do not modify the shared Button base to compensate for a page-level stretch problem.

## Icon-Led Actions

When a page is text-heavy or repeated actions need faster scanning:

1. Read
   [`../ui-ux-pro-max/references/icon-led-interface.md`](../ui-ux-pro-max/references/icon-led-interface.md)
   completely.
2. Read `components.json` and use its configured `iconLibrary`; this project uses Lucide.
3. Use icon-only Buttons for familiar repeated actions only. Set an accessible name and provide a
   Tooltip.
4. Use icon + label for primary, unfamiliar, destructive, expensive, or server-state-changing
   actions.
5. Use `size="icon"`, `icon-sm`, or another supported icon size only for icon-only Buttons.
6. Add `data-icon="inline-start"` or `inline-end` for icons beside labels.
7. Do not add decorative icons to every heading, field label, or paragraph.

## Entity Detail Hierarchy

When implementing account, customer, project, workflow, order, device, or other detail pages:

1. Read
   [`../ui-ux-pro-max/references/entity-detail-hierarchy.md`](../ui-ux-pro-max/references/entity-detail-hierarchy.md)
   completely.
2. Build one flat entity header containing identity, status, compact metadata, and an Actions
   trigger. Do not wrap this page chrome in a Card.
3. Put secondary entity commands in `DropdownMenu`; keep destructive commands explicit and
   confirmed.
4. Put route-backed `Tabs` or equivalent local navigation immediately below the header and a
   `Separator`.
5. Do not duplicate tab destinations as header Buttons.
6. Use Cards only for content groups below the local navigation.
7. Attach each action to the entity, section, row, field, or form it changes.
