...
describe('Admin RBAC (e2e)', () => {
	...
	// Credentials are owned by the API environment, just like production bootstrap.
	...
	describe('User Management', () => {
		...
		it('should filter by role and apply the requested user sort', async () => {
			const res = await request(app.getHttpServer())
				.get('/v1/admin/users?roleId=admin&sort=email:asc')
				.set('Authorization', `Bearer ${superAdminToken}`);

			expect(res.status).toBe(200);
			const items = res.body.data.data;
			expect(items.every((user: any) =>
				user.userRoles.some((userRole: any) => userRole.role.id === 'admin')
			)).toBe(true);
			const emails = items.map((user: any) => user.email);
			expect(emails).toEqual([...emails].sort((left, right) => left.localeCompare(right)));
		});
	});
	...
	describe('Role & Permission Management', () => {
		...
		it('should assign permission to Editor role', async () => {
			...
			const perms = Array.isArray(permRes.body.data)
				? permRes.body.data
				: permRes.body.data.data;
		...
		it('should create Editor User, Login, and Access Allowed Resources', async () => {
			...
			if (profileRes.status !== 200) {
				console.error(
					...
					JSON.stringify(profileRes.body, null, 2)
				);
			}
