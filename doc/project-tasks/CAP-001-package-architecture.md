...
> **Release**: Direct integration to `dev` and `main` (2026-09-02)
...
- 2026-09-02：將 CAP-001 直接整合進 `dev` 時保留既有 OAuth、Remote MCP、Task Runtime、Service UI 與 migration 0016／0017，並遷移到新的 capability package 分層。
- 2026-09-02：修正 MCP 與 OAuth composition，統一由 API `ConfigModule`／`ConfigService` 注入窄設定，不再於 MCP、OAuth 或 `main.ts` 直接讀取 `process.env`；MCP protocol/config tests 13/13 與 API e2e 16/16 通過。
- 2026-09-02：完成合併後 migration、dependency checks、unit/regression、build、lint、frozen lockfile 與實際 API／MCP／Web production-preview startup smoke checks。

## Handoff

- **Commit/PR**: feature commit `07f0797`；Owner-authorized direct `dev`／`main` integration
...
- **Validation**: `bun run check`、`bun run test`、`bun run build`、`bun run lint`、`bun run deps:audit`、`bun install --frozen-lockfile`、API unit 56 passed／1 skipped、API e2e 16/16、PostgreSQL migration、MCP initialize、Web production preview、Storybook 38 個 component tests、Storybook static build、Admin dark/editor layout 瀏覽器回歸與 `git diff --check` 通過。
- **Known issues**:
  - OpenAPI runtime schema 與 TypeScript interface 的完全同步不在本 Work Item 內。
  - Web 維持既有 15 個 `svelte-check` warnings 與 25 個 ESLint warnings，皆為 0 errors。
...
- **Next action**: 獨立 review 與 OpenAPI typed client 另開 Work Item。
