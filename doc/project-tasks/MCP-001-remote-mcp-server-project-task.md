...
- **Summary**: Compose two standard Streamable HTTP MCP endpoints in `apps/api`: anonymous `/mcp/public` and OAuth-protected `/mcp/private`. Use CMS as the first real capability, keep `@platform/nest-infra-mcp-server` independent from CMS, and let the app inject `CmsService` from `@platform/nest-content-cms` directly when registering tools.
- **Key decisions**:
  ...
  - Do not add `apps/mcp`, local/stdio MCP, or `@platform/nest-content-cms-mcp`.
  - `apps/api` is the composition root. It imports the generic MCP adapter and `@platform/nest-content-cms`, injects `CmsService`, and registers the tool handlers.
  - Split the current misleading `@platform/types-content` package so Nest-specific controllers, module, service, HTTP DTOs, and validation live in `@platform/nest-content-cms`; keep only genuine framework-neutral CMS contracts or pure logic in `@platform/types-content`.
- **Assumptions**:
...
- **Risks and acceptance**:
  - The official MCP TypeScript SDK v2 is new. Pin a stable release, hide it behind `@platform/nest-infra-mcp-server`, and cover the public transport contract with integration tests.
...
- [x] `@platform/nest-infra-mcp-server` has no dependency on CMS; no `nest-cms-mcp` package or separate MCP app is introduced.
- [x] `@platform/nest-content-cms` exports `CmsModule` and `CmsService`; `apps/api` injects that service directly for REST and MCP composition.
...
- `@platform/nest-infra-mcp-server`, standard Streamable HTTP handling, registry/server factory, endpoint error mapping, and protected-resource metadata helpers.
- `@platform/types-content` framework-neutral query/result contracts and `@platform/nest-content-cms` Nest/Drizzle implementation.
...
- [x] PR and required CI ([#15](https://github.com/doremi31618/monorepo-system-template/pull/15))
- [x] Release note and release approval
...
- 2026-09-01: Approved direct injection of `@platform/nest-content-cms` `CmsService` from `apps/api`; explicitly rejected a `nest-cms-mcp` package.
...
- 2026-09-01: Added `@platform/types-content` framework-neutral contracts, `@platform/nest-content-cms`, generic `@platform/nest-infra-mcp-server`, and app-owned public/private CMS tool registration.
